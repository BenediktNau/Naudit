...
**Important:** mount a persistent volume at `/data` — the SQLite database lives there.
Without it, accounts and review history are lost on every redeploy.
...
- **Persistence** is EF Core + SQLite (`src/Naudit.Infrastructure/Data/`); the schema is
  applied via `Database.Migrate()` at startup (only when the UI is enabled).
