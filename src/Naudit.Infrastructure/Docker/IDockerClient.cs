...
/// <summary>Volume-Felder sind optional: die Session-Sandbox mountet ihr Credential-Volume.
/// Environment/Entrypoint sind ebenfalls optional — ohne sie verhält sich RunDetachedAsync
/// wie ein schlichtes "docker run -d".</summary>
public sealed record ContainerRunSpec(string Name, string Image, string? VolumeName, string? VolumeTarget,
    IReadOnlyList<string> Command)
{
    /// <summary>Env des Containers; null = nichts Zusätzliches.</summary>
    public IReadOnlyDictionary<string, string?>? Environment { get; init; }

    /// <summary>Überschreibt den ENTRYPOINT des Images (z. B. ["sleep","infinity"] für einen
    /// passiven Container, der nur per docker exec benutzt wird). null = ENTRYPOINT des Images gilt.</summary>
